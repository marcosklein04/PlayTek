...
from trivia.models import QuestionSet
from trivia.services import pick_question_set_for_session  # ideal: service para no ensuciar views
...
def iniciar_juego(request, slug: str):
    ...
    with transaction.atomic():
        ...
        if juego.slug == "trivia":
            qs = pick_question_set_for_session(user=request.user, juego=juego)
            if not qs:
                return JsonResponse(
                    {"error": "sesion_sin_question_set"},
                    status=409
                )
            sesion.question_set = qs
            sesion.save(update_fields=["question_set"])
